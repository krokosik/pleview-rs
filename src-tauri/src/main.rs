...
#[tauri::command]
fn get_initial_data(engine: tauri::State<'_, EngineState>) -> Result<(cross_section::CrossSection, Vec<Vec<f64>>), String> {
...
#[tauri::command]
fn update_cross_section(direction: usize, pixel: usize, engine: tauri::State<'_, EngineState>) -> Result<Vec<f64>, String> {
    let mut engine = engine.0.lock().unwrap();
    let direction = if direction == 0 { enums::Direction::X } else { enums::Direction::Y };
...
fn main() {
    ...
    tauri::Builder::default()
        .manage(state)
        .invoke_handler(tauri::generate_handler![get_initial_data, update_cross_section])
